        ...
        <VDataTableServer v-model:items-per-page="options.itemsPerPage" v-model:page="options.page" :items="contracts"
          ...
          <div class="py-3">fullName</div>
            ...
            <template #item.status="{ item }">
                <VChip :color="item.status == 'inProgress' ? 'primary' : item.status == 'active' ? 'success' : 'error'"
                    :class="`text-${item.status == 'inProgress' ? 'primary' : item.status == 'active' ? 'success' : 'error'}`"
                    size="small" class="font-weight-medium">
                    {{ item.status == 'inProgress' ? 'In Progress' : item.status == 'active' ? 'Active' : 'Cancelled' }}
                </VChip>
            </template>
            ...
            <template #item.title="{ item }">
                <span class="text-sm">
                    {{ item.contractId?.contractDetails?.title }}
                </span>
            </template>

            <template #item.contractDetails="{ item }">
                <span class="text-sm">
                    {{ item.contractId?.contractDetails?.dateOfAgreement }}
                </span>
            </template>
            ...
            <template #item.paymentPlan="{ item }">
                <div class="d-flex flex-column">
                    <span class="text-sm">
                        {{ item.contractId?.contractDetails?.paymentPlan }}
                    </span>
                    <small>
                        {{ item.pricing }}
                    </small>
            ...
            <template #bottom>
                ...
                <div class="d-flex justify-end gap-x-6 pa-2 flex-wrap">
                    <div class="d-flex align-center gap-x-2 text-sm">
                        Rows Per Page:
                        <VSelect v-model="options.itemsPerPage" class="per-page-select text-high-emphasis"
                            variant="plain" density="compact" :items="[10, 20, 25, 50, 100]" />
                    </div>

                    <div class="d-flex gap-x-2 align-center me-2">
                        <VBtn icon="mdi-chevron-left" class="flip-in-rtl" variant="text" density="comfortable"
                            color="default" :disabled="options.page <= 1"
                            @click="options.page <= 1 ? (options.page = 1) : options.page--" />

                        <VBtn icon="mdi-chevron-right" class="flip-in-rtl" density="comfortable" variant="text"
                            color="default" :disabled="options.page >= Math.ceil(totalUsers / options.itemsPerPage)
                                " @click="
                ...
                <VCardText>
                    <VRow>
                        ...
                        <VCol cols="8">
                            ...
                            <v-textarea v-model="text" label="Comments" row-height="25" rows="5" variant="outlined"
                                auto-grow shaped class="pt-8"></v-textarea>
...
const listContracts = () => {
    ...
    axios
        .get(`influencer/list/projects?sortBy=createdAt`)
        .then((response) => {
            console.log("useradsfasdf", response.data.data);
...
const viewDetails = (item) => {
    ...
    router.push({ path: `/project-details/${item._id}` });
};
</script>
