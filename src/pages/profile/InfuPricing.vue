<template>
    <div class="mt-8">
        <div class="d-flex justify-space-between align-center">
            ...
            <div>
                <v-btn-toggle v-model="toggle" color="primary" variant="outlined" mandatory>
        ...
        <VRow class="ml-4">
            <VCol cols="12" md="4" v-for="pkg in selectedPkg" :key="pkg">
                <v-card class="mt-4 cursor-pointer" :class="pkg.id != currentPkg ? 'selected-card' : 'pricing-card'"
                    min-height="400" variant="outlined" color="black" max-width="300" v-model="currentPkg"
                    @click="selectPkg(pkg.id)">
                    <v-card-text>
                        <div>
                            <span class="f-18" :class="pkg.id != currentPkg ? '' : 'selected-card-text'">
                                {{ pkg.name }}
                            </span>
                            <div class="my-3">
                                <span class="font-weight-bold f-24">{{ pkg.price }}</span>/Recording
                            </div>
